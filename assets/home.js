...
.then(datosEventos => {
    data = datosEventos
    let dentroDeData = data.events
        createCheckbox(dentroDeData, $check)
        printCards(dentroDeData, $cards)
...
function filters( ){
    ...
    let filterPerCheck = data.events.filter(event => checked.includes(event.category) || checked.length === 0 )
