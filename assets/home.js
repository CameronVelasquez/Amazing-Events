 ...
 function crearCards( lista){
     ...
     for (let recorrido of lista){
        let template =
                    ...
                    <div class="card-body">
                    <h5 class="card-title width">${recorrido.name}</h5>
                    <hr>
...
renderTemplate (crearCards(data.events), $cards) 

//generar checks en js
//variable
const sinRepetidos = []
const categorias = data.events.map(events => events.category) //nombre de las categorias, aun repetidas en este punto.
...
function searchMovies(inputBusqueda, vaArecorrerDataEvents){
  const filterMovies = vaArecorrerDataEvents.filter(propiedadesDeEvents => {
    return propiedadesDeEvents.name.toLowerCase().startsWith(inputBusqueda.value.toLowerCase())
  })
